...
FROM surf_dv_logs WHERE SURF_DV_LOG_ID = ?`
	...
	files, err := getFiles(row.FirstFile, row.LastFile)
...
func getFiles(firstFile, lastFile string) ([]string, error) {
	if firstFile == "" {
		return nil, nil
	}

	if firstFile == lastFile {
		return []string{firstFile}, nil
	}

	firstPrefix, firstNum, firstExt, err := parseFileSuffix(firstFile)
	if err != nil {
		return nil, err
	}

	lastPrefix, lastNum, lastExt, err := parseFileSuffix(lastFile)
	if err != nil {
		return nil, err
	}

	if firstPrefix != lastPrefix || firstExt != lastExt {
		return nil, fmt.Errorf("file names do not match: %q, %q", firstFile, lastFile)
	}

	if lastNum < firstNum {
		return nil, fmt.Errorf("last file number %d is before first file number %d", lastNum, firstNum)
	}

	files := make([]string, 0, lastNum-firstNum+1)
	for n := firstNum; n <= lastNum; n++ {
		files = append(files, fmt.Sprintf("%s%03d%s", firstPrefix, n, firstExt))
	}

	return files, nil
}

func parseFileSuffix(fileName string) (prefix string, num int, ext string, err error) {
	...
	numPart, fileExt, found := strings.Cut(last, ".")
	if !found {
		return "", 0, "", fmt.Errorf("invalid file name %q", fileName)
	}
	...
	if err != nil {
		return "", 0, "", fmt.Errorf("invalid file number %q: %w", numPart, err)
	}
